...
public class TestMetadataTables extends SparkExtensionsTestBase {
  ...
  public void testSnapshotReferencesMetatable() throws Exception {
    ...
    List<Row> branches = spark.sql("SELECT * FROM " + tableName + ".refs WHERE type='BRANCH'").collectAsList();
    Assert.assertEquals("Refs table should return 2 branches", 2, branches.size());
    List<Row> tags = spark.sql("SELECT * FROM " + tableName + ".refs WHERE type='TAG'").collectAsList();
    Assert.assertEquals("Refs table should return 1 tag", 1, tags.size());

    // Check branch entries in refs table
    List<Row> mainBranch =
        spark.sql("SELECT * FROM " + tableName + ".refs WHERE name = 'main' AND type='BRANCH'").collectAsList();
    ...
    List<Row> testBranch =
        spark.sql("SELECT * FROM " + tableName + ".refs WHERE name = 'testBranch' AND type='BRANCH'").collectAsList();
    ...
    List<Row> testTag =
        spark.sql("SELECT * FROM " + tableName + ".refs WHERE name = 'testTag' AND type='TAG'").collectAsList();
    ...
    List<Row> testTagProjection = spark
                    .sql("SELECT name,type,snapshot_id,max_reference_age_in_ms,min_snapshots_to_keep FROM " + tableName + ".refs where type='TAG'")
                    .collectAsList();
    ...
    Assert.assertEquals(Long.valueOf(50), testTagProjection.get(0).getAs("max_reference_age_in_ms"));
    Assert.assertNull(testTagProjection.get(0).getAs("min_snapshots_to_keep"));

    List<Row> mainBranchProjection =
        spark.sql("SELECT name,type FROM " + tableName + ".refs WHERE name = 'main' AND type='BRANCH'").collectAsList();
    ...
    Assert.assertThrows(IllegalArgumentException.class, () -> mainBranchProjection.get(0).getAs("snapshot_id"));

    List<Row> testBranchProjection =
        spark.sql("SELECT type,name,max_reference_age_in_ms,snapshot_id FROM " + tableName + ".refs WHERE name = 'testBranch' AND type='BRANCH'").collectAsList();
    ...
    Assert.assertEquals(Long.valueOf(10), testBranchProjection.get(0).getAs("max_reference_age_in_ms"));
    Assert.assertThrows(IllegalArgumentException.class, () -> testBranchProjection.get(0).getAs("min_snapshots_to_keep"));
    Assert.assertThrows(IllegalArgumentException.class, () -> testBranchProjection.get(0).getAs("max_snapshot_age_in_ms"));
  }
